        ...
        <div class="layout-page">
          ...
          <div class="content-wrapper">
            ...
            <div class="container-xxl flex-grow-1 container-p-y">
              <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Detail</span> Penyewaan Indekos</h4>
                    ...
                    <div class="col-xxl">
                        <div class="card mb-4">
                            <div class="card-header d-flex align-items-center justify-content-between">
                                <h5 class="mb-0">Detail Penyewaan</h5>
                                <a href="{{ URL::previous() }}" class="btn btn-md btn-outline-secondary float-end"><i class="bx bx-undo"></i> Kembali</a>
                            </div>
                            <div class="card-body">
                            <form method="POST" action="{{ url('admin/penyewaan/'. $datapenyewaan->id_penyewaan)}}" enctype="multipart/form-data">
                              @method('patch')
                              @csrf
                                <div class="row mb-3">
                                <label class="col-sm-2 col-form-label" for="basic-default-name">Kamar</label>
                                <div class="col-sm-10">
                                    <select id="id_kamar" name="id_kamar" class="form-select @error('id_kamar') is-invalid @enderror">
                                      @foreach ($datakamar as $kamar)
                                        @if($datapenyewaan->id_kamar == $kamar->id_kamar)
                                            <option selected value="{{$kamar->id_kamar}}">{{ $kamar->nama_kamar }}</option>
                                        @endif
                                ...
                                <div class="row mb-3">
                                <label class="col-sm-2 col-form-label" for="basic-default-name">Nama Penghuni</label>
                                <div class="col-sm-10">
                                    <select id="id_user" name="id_user" class="form-select @error('id_user') is-invalid @enderror">
                                      @foreach ($datapenghuni as $penghuni)
                                      @if($datapenyewaan->id_user == $penghuni->id_user)
                                            <option selected value="{{$penghuni->id_user}}">{{ $penghuni->nama_user }}</option>
                                        @endif
                                        <option value="{{$penghuni->id_user}}">{{ $penghuni->nama_user }}</option>  
                                      @endforeach
                                    </select>
                                    @error('id_user')
                                    <div class="invalid-feedback">
                                ...
                                <div class="row mb-3">
                                <label class="col-sm-2 col-form-label" for="basic-default-company">Mulai Sewa</label>
                                <div class="col-sm-10">
                                    <input type="date" class="form-control @error('mulai_sewa') is-invalid @enderror" id="mulai_sewa" name="mulai_sewa" value="{{$datapenyewaan->mulai_sewa}}"/>
                                    @error('mulai_sewa')
                                    <div class="invalid-feedback">
                                          {{ $message }}
                                    </div>
                                ...
                                <div class="row mb-3">
                                <label class="col-sm-2 col-form-label" for="basic-default-company">Akhir Sewa</label>
                                <div class="col-sm-10">
                                    <input type="date" class="form-control @error('akhir_sewa') is-invalid @enderror" id="akhir_sewa" name="akhir_sewa" value="{{$datapenyewaan->akhir_sewa}}" />
                                    @error('akhir_sewa')
                                    <div class="invalid-feedback">
                                          {{ $message }}
                                    </div>
                                ...
                                <div class="row mb-3">
                                <label class="col-sm-2 col-form-label" for="basic-default-phone">Jumlah Biaya</label>
                                <div class="col-sm-10">
                                    <input type="number" id="jumlah" name="jumlah" class="form-control @error('jumlah') is-invalid @enderror" value="{{$datapenyewaan->jumlah}}" />
                                    @error('jumlah')
                                    <div class="invalid-feedback">
                                          {{ $message }}
                                    </div>
                                ...
                                <div class="col-sm-10">
                                    <button type="submit" class="btn btn-primary"><i class="bx bx-plus"></i> Edit</button>
                                </div>
                                </div>
                            </form>
                            </div>
                        ...
                        <div class="card">
                            <h5 class="card-header">Ingatkan Penghuni</h5>
                            <div class="card-body">
                            <div class="mb-3 col-12 mb-0">
                                <div class="alert alert-secondary">
                                <h6 class="alert-heading fw-bold mb-1">Ingatkan penghuni masa sewa sebentar lagi selesai?</h6>
                                <p class="mb-0">Dengan menekan tombol dibawah maka penghuni akan mengkonfirmasi apakah lanjut sewa atau tidak</p>
                                </div>
                            </div>
                            <form method="POST" action="{{ url('/admin/penyewaan/ingatkan/' . $datapenyewaan->id_penyewaan) }}" method="post" class="d-inline">
                                @method('patch')
                                @csrf
                                <div class="form-check mb-3">
                                  <input class="form-check-input" type="checkbox" name="validasi" id="validasi" required="">
                                  <label class="form-check-label" for="">Ingatkan penghuni untuk konfirmasi kelanjutan Sewa</label>
                                </div>
                                <button type="submit" class="btn btn-info">Ingatkan Penghuni</button>
                            </form>
                            </div>
                        </div>
                    </div>
